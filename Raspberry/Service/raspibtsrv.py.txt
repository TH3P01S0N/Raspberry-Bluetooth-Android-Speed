        ...
        try:
            ...
            while True:
                ...
                try:
                        ...
                        respint=randint(0,40)
                        accel_x=accel_data['x']
                        accel_y=accel_data['y']
                        accel_z=accel_data['z']
                        accel_total= sqrt(accel_x**2 + accel_y**2 + accel_z**2) - 9.6
                        response = ("%.4f" %round (float(accel_total),4))
                        #response = str(respint)
                        time.sleep(1)
                        client_sock.send(str(response))
                        print "Sent back [%s]" % str(response)
                except IOError:
                        break
